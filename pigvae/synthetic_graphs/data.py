...
class RandomGraphDataset(Dataset):
    def __init__(self, n_min=12, n_max=20, samples_per_epoch=100000, pyg=False, **kwargs):
        super().__init__()
        self.n_min = n_min
        self.n_max = n_max
        self.samples_per_epoch = samples_per_epoch
...
class GraphDataModule(pl.LightningDataModule):
    def __init__(self, graph_family, graph_kwargs=None, samples_per_epoch=100000, batch_size=32,
                 distributed_sampler=True, num_workers=1, use_saved_graphs=False, save_dir=None):
        super().__init__()
    ...
    def on_epoch_end(self):
        self.save_sampled_graphs(epoch_samples=100)

    def on_train_end(self):
        save_dir = "/home/shuaiqun/Graph-instance-generation/pigvae-main/saved_training_samples_mix_6000_1"
    ...
    def save_additional_samples(self, remaining, save_dir, start_index):
        ...
        for idx in indices:
            ...
            file_path = os.path.join(save_dir, f"graph_{start_index}.gpickle")
            self.train_dataset.save_graph(graph, file_path)
            start_index += 1
            
    def save_sampled_graphs(self, epoch_samples=100):
        ...
        if hasattr(self.train_dataset, 'save_graph'):
            ...
            if saved_count < total_required:
                ...
                for idx in indices:
                    graph = self.train_dataset[idx]
                    file_path = os.path.join(save_dir, f"graph_{saved_count}.gpickle")
                    self.train_dataset.save_graph(graph, file_path)
                    saved_count += 1
            else:
                # Random replacement process
                ...
                for file_idx, data_idx in zip(file_indices, new_indices):
                    graph = self.train_dataset[data_idx]
                    file_path = os.path.join(save_dir, f"graph_{file_idx}.gpickle")
                    self.train_dataset.save_graph(graph, file_path)
...
class SavedGraphDataset(Dataset):
    def __init__(self, save_dir, sample_size=6000):
        ...
        self.file_names = random.sample(all_file_names, sample_size)
